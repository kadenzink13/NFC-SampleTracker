...
def update_scan_status(uid, is_new):
    ...
    with open(SCAN_STATUS_FILE, "r") as file:
        reader = csv.DictReader(file)
        for row in reader:
            if row["Sample UID"] == uid and row["new"] == "True":
                csv_says_new = True
                break

    if csv_says_new:
        print("Waiting for web app to complete data entry...")
        while True:
            done = False
            with open(SCAN_STATUS_FILE, "r") as file:
                reader = csv.DictReader(file)
                for row in reader:
                    if row["Sample UID"] == uid:
                        if row["new"] == "False":
                            ...
                            print("Waiting for data entry...")
                            time.sleep(1)
                            break
                else:
                    # UID was removed from file
                    break
            if done:
                break
        ...
        # Reset scan status for this UID
        for row in rows:
            if row["Sample UID"] == uid:
                row["Host scan"] = "False"
                row["new"] = "False"
